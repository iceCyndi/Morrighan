...
public class ControlStick : MonoBehaviour
{
    ...
    public float rangeStick = 0.08f;//stick可以移动范围半径
    ...
    void Awake()
    {
        ...
        controllerButton = transform.FindChild("ControllerButton");
        controllerBg = transform.FindChild("ControllerBG");
        bgOriginalPos = transform.FindChild("ControllerBGOriginalPos");
    ...
    void InitSize()
    {
        ...
        actualCenterBg.x = controllerBg.localPosition.x * RelatedCamera.GetScreenWidth() / RelatedCamera.orthographicSize;
        actualCenterBg.y = controllerBg.localPosition.y * RelatedCamera.GetScreenHeight() / (RelatedCamera.orthographicSize / RelatedCamera.aspect);
        ...
        float aspect = RelatedCamera.GetScreenHeight() / 720;
        rectCanTouch = new Rect(0, RelatedCamera.GetScreenHeight() - box.size.y * aspect, box.size.x * aspect, box.size.y * aspect);
        isFirstLanuch = false;
        isDraging = false;
    }
    ...
    void Update()
    {
        ...
        if (isDraging)
        {
            ...
            controllerButton.localPosition = new Vector3((actualRectStick.xMax + actualRectStick.xMin) * 0.5f *
                RelatedCamera.orthographicSize / RelatedCamera.GetScreenWidth(), (RelatedCamera.GetScreenHeight() - (actualRectStick.yMin + actualRectStick.yMax) * 0.5f) *
                    (RelatedCamera.orthographicSize / RelatedCamera.aspect) / RelatedCamera.GetScreenHeight(), 0);


            controllerBg.localPosition = new Vector3((actualRectBg.xMax + actualRectBg.xMin) * 0.5f *
                RelatedCamera.orthographicSize / RelatedCamera.GetScreenWidth(), (RelatedCamera.GetScreenHeight() - (actualRectBg.yMin + actualRectBg.yMax) * 0.5f) *
                    (RelatedCamera.orthographicSize / RelatedCamera.aspect) / RelatedCamera.GetScreenHeight(), 0);
        }
