...
public class MogoMainCamera : MonoBehaviour
{
    ...
    public const int LOCK_SIGHT2 = 4; //定桩
    public const int VICTORY = 5;//人物胜利特写
    public const int LOCK_SIGHT3 = 6;//看着人，同时人，怪，摄像机3者同一个平面
    ...
    private void GetRotationAndPositon(Transform t, float dis, float rotationY, float rotationX, out Vector3 position, out Vector3 rotation)
    {
        ...
        Transform slot_camera = t.FindChild("slot_camera");
        if (slot_camera != null)
        {
    ...
    /// <param name="_moveTime">positon移动时间</param>
    /// <param name="_rotateTime">rotation转向时间</param>
    ...
    /// </summary>
    /// <param name="_target">目标</param>
    /// <param name="distance">距离目标距离</param>
    /// <param name="rotationY">绕目标y轴角度</param>
    /// <param name="rotationX">绕目标x轴角度</param>
    /// <param name="_moveTime">pos移动时间</param>
    /// <param name="_rotateTime">rotation转向时间</param>
        ...
        float _length, Transform _target, Action _onDone = null)
    {
        ...
        Transform slotCamera = _target.FindChild("slot_camera");
        if (slotCamera != null)
        {
            _target = slotCamera;
    ...
    public void PlayVictoryCG()
    {
        ...
        int weaponType = 1;// InventoryManager.Instance.GetCurrentWeaponType();
        ...
        Animation ani = MogoWorld.thePlayer.Transform.Find(data.path).animation;
        ani.Stop();
        ani.Play();
    ...
    private void DealWithShiedObjects(RaycastHit[] hits)
    {
        ...
        while (i < hits.Length)
        {
            ...
            if (!withoutTestObjectTags.Contains(name))
            {
                ...
                Material m = obj.renderer.material;
    ...
    private void ResetLastObjects()
    {
        ...
        {
            if (go == null) continue;
            Material m = go.renderer.material;
