...
public class CreateCharacterDetailUIJobAttr : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        m_lblAttrName = m_myTransform.FindChild("CreateCharacterDetailUIJobAttrText").GetComponentsInChildren<UILabel>(true)[0];
        for (int i = 0; i < 6; ++i)
        {
            m_arrSpSignFG[i] = m_myTransform.FindChild("CreateCharacterDetailUIJobAttrSignList/CreateCharacterDetailUIJobAttrSign" + i + "FG").GetComponentsInChildren<UISprite>(true)[0];
        }
    }
